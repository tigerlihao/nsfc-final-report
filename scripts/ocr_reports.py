...
def find_pages(project_dir: str) -> List[str]:
    ...
    pages = [f for f in files if f.lower().endswith(IMAGE_EXTS) and f.startswith('page_')]
...
def ocr_dir(project_dir: str, out_path: str, header: str = None, lang: str = None) -> None:
...
def main():
    parser = argparse.ArgumentParser(description='OCR NSFC report images in a project directory and combine to text')
    parser.add_argument('project_dir', help='path to project directory containing page_###.png/jpg')
    parser.add_argument('--out', '-o', default=None, help='output text file (default: <project_dir>/report.txt)')
    parser.add_argument('--header', '-H', default=None, help='optional header text to prepend')
    parser.add_argument('--lang', '-l', default=None, help='tesseract language code to pass as -l (e.g. chi_sim)')
